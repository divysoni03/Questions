...
bool isPossible(vector<int>& arr, int n, int m, int mid) {
    ...
    int pageSum = 0;
    for(int i = 0; i < n; i++) {
        // If adding this book to the current student's allocation doesn't exceed `mid`
        if((pageSum + arr[i]) <= mid) {
            pageSum += arr[i];
        } else {
            // Allocate books to a new student
            student++;
            // Check if we exceed the number of students or a single book exceeds `mid`
...
int findPages(vector<int>& arr, int n, int m) {
    // Edge case: If number of books is less than the number of students
    if(n < m) return -1;
    
    int s = 0, e = 0, ans = -1;
    for(int i = 0; i < n; i++) {
        e += arr[i]; // Calculate the sum of all pages
    }
    
    while(s <= e) {
        int mid = s + (e - s) / 2; // Calculate mid value
        if(isPossible(arr, n, m, mid)) {
            ans = mid; // Possible solution found
